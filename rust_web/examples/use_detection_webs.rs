...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rg_graph = make_rg(graph);
    let detection_webs = DetectionWebs::new(rg_graph.clone());
    let webs = detection_webs.get_detection_webs()?;
    let counter = 0;
    for web in webs {
        ...
        let output_path = format!("{}/{}", output_dir, output_filename);
         ...
         let dot_content = graph_visualizer::to_dot_with_positions(&detection_webs.graph, Some(&web), false);
         std::fs::write(&dot_path, dot_content)
             .map_err(|e| format!("Failed to write DOT file: {}", e))?;
